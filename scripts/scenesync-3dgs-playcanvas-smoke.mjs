...
try {
  ...
  assert(result.xrLocomotionSpeed === 1.8, 'PlayCanvas XR locomotion speed changed');
  assert(result.xrVerticalLocomotionSpeed === 1.2, 'PlayCanvas XR vertical speed changed');
  assert(result.xrTurnSpeedDegrees === 105, 'PlayCanvas XR turn speed changed');
  assert(/left-stick/iu.test(result.xrLocomotion), 'PlayCanvas PICO locomotion is unavailable');
  assert(/A up \/ B down/iu.test(result.xrLocomotion), 'PlayCanvas PICO vertical locomotion is unavailable');
